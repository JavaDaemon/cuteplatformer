...
public class JumpScreen extends GameScreen {
	...
	public void render(float delta) {
		...
		batch.draw(
				actor.getTexture(), 
				actor.getPosition().x*Settings.PIXELS_PER_BLOCK_SCALED-(int)(camera.getOffset()*Settings.PIXELS_PER_BLOCK_SCALED), 
				actor.getPosition().y*Settings.PIXELS_PER_BLOCK_SCALED, 
				Settings.PIXELS_PLAYER_WIDTH_SCALED, 
				Settings.PIXELS_PLAYER_HEIGHT_SCALED);
		
		Vector2 position = actor.getPosition();
		font.draw(batch, "position.x: "+position.x, 100, 100);
		font.draw(batch, "position.y: "+position.y, 100, 70);
